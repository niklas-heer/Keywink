...
**Top Edge** is the default launcher theme: a compact, native-material panel below the menu bar. It uses one softly rounded surface with consistent spacing on built-in and external displays, keeping clear of the camera without attaching to it. Shortcut hints are built in, and the panel resizes as you navigate groups. It follows light/dark appearance and the system's reduced-motion/transparency settings. Choose **Settings → General → Theme → Top Edge** if you previously selected another theme; the original themes remain available.
